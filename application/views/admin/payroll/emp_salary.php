...
<div id="wrapper">
    <div class="content">
        ...
        <div class="row">
            <center>
                <h3>Employee Monthly Salary Report </h3>
            </center>
        </div>
        <br>
        <hr class="hr-panel-heading">
        ...
        <div class="dropdown bootstrap-select show-tick">
            <input type="button" class="btn btn-success" onclick="printDiv('printData')" value="Salary Print" />
        </div>
        <div class="dropdown bootstrap-select show-tick">
                ...
                <table id="testTable" class="table table-responsive">
                    <thead>
                        <tr>
                            ...
                            <td><strong>Day Deduction(Salary)</strong></td>
        ...
        $attendanceDate = $query->result(); */
                                    ...
                                    $this->db->where("date(tblleaveapplication.application_status)", 2);
                                    $query = $this->db->get();
                                    ...
                                    $this->db->where('date(LogDate)', $dates);
                                    $this->db->where('staffId', $v_employee['staffid']);
                                    $absentdataaa = $this->db->get()->result();
                                    //echo $absentdataaa[0];

                                    /* $this->db->select('*');
		$this->db->where('date(quota)', $dates);
		//$this->db->where('date(leave_end_date) <=', $date);
		$holithisday = $this->db->get('tblholidays')->result(); */

                                    $get_query = $this->db->query("SELECT `name` FROM `tblholidays` WHERE `quota` = '" . $dates . "' OR `leave_end_date` = '" . $dates . "' OR (`quota` < '" . $dates . "' AND`leave_end_date` > '" . $dates . "')");
                                    $holithisday = $get_query->result();
			...
			$sumofmornmissed = $sumofmornmissed + 1;
                                    ...
                                    if (empty($absentdataaa) && empty($holithisday) && $leaveDate[0] == "" && date("l", strtotime($dates)) != "Sunday") {
                                        //$datarule = $datarule+1;
                                    }
        ...
        $preDate = $diff->format("%a"); */
		  ...
		  $datarule = 0.5;
                                ...
                                if ($v_employee['staffid'] == 113) {
                                    $datarule = -1;
                                }
                                if ($v_employee['staffid'] == 28) {
                                    $sumofmornmissed = 0;
                                    $sumofmornhalf = 0;
                                    //$deductMissed = 0;
                                    $totDeductDays = 0;
                                ...
                                /* if($v_employee['staffid'] == 98){
		  $datarule = 1;
	  }
	  if($v_employee['staffid'] == 94){
		  $datarule = 0.5;
	  }
	  if($v_employee['staffid'] == 110){
		  $datarule = 1;
	  }
	  ...
	  if($v_employee['staffid'] == 108){
		  ...
		  $datarule = 1;
                                ...
                                $totallwpabsent = $totallwp + $sumoftotalabsent + $preDate + $datarule;

                                $totaldayded = $totDeductDays + $totallwpabsent + ($acthldcount / 2);
                                if ($v_employee['staffid'] == 110) {
                                    $totaldayded = "1.5";
                                }
                                if ($v_employee['staffid'] == 104) {
                                    $datarule = 0.5;
                                }
