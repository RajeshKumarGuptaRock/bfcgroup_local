...
/*	function getCompletedProjects(){
	...
	function get_inprogress_Projects(){
		...
		if (is_admin() || is_headtrm()) {
			...
			//  echo $this->db->last_query();
			//  die();
			 $query2 = $this->db->query("SELECT * FROM `tblstaff` JOIN `tblleads` ON `tblleads`.`assigned` = `tblstaff`.`staffid` WHERE `tblstaff`.`pre_pm_assign_to` = '".$pm_id."' AND `tblleads`.`pm_project_status` = '2' AND tblleads.lead_booking_amount_date < tblstaff.pm_assign_date ORDER BY `tblleads`.`lead_pm_takeup_date` DESC;")->result();
			 $query = array_merge($query1,$query2);
			return $query;
		}
	}
