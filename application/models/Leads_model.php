    ...
    function upload_leads_26bkp()
    {
        ...
        while ($csv_line = fgetcsv($fp, 10240)) {
            ...
            if ($count == 1) {
                ...
                }else if($findslash1 >=1){
                    $seprate_date1=explode(",",$csv_line[8]);
                    foreach($seprate_date1 as $key => $value){
                    $this->db->select('id,phonenumber');
                    $this->db->like('phonenumber', $value);
                    $this->db->from('tblleads');
                    $query = $this->db->get();
...
// return  "test".$services.$book_type;die;
    ...
    public function getlead_multi_create_Data($leadId){
    ...
    public function getTransferRecordCount($search = '',$search_cat='',$start_date = '',$end_date='',$staff_name = '') {
    ...
    public function transferLeadsData($limit, $start,$search_text='',$search_cat='',$start_date = '',$end_date='', $staff_name = ''){
        $useraid = $this->session->userdata('staff_user_id');
        // print_r($useraid);die;
        $role = get_imp_role();
        $arr = herapermission();
